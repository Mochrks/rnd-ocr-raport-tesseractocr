...
from app.services.ocr_service import perform_ocr_and_extract, perform_pdf_ocr_and_extract, debug_ocr_raw_text
...
async def debug_ocr(documentId: str):
    ...
    Use this to troubleshoot what Tesseract actually reads from your image.
    """
    if documentId not in DOCUMENTS_STORE:
        raise HTTPException(status_code=404, detail="Document not found")
...
async def confirm_results(request: ConfirmRequest):
    if request.documentId not in DOCUMENTS_STORE:
        raise HTTPException(status_code=404, detail="Document not found")
        
    DOCUMENTS_STORE[request.documentId]["extracted_data"] = [item.dict() for item in request.subjects]
    DOCUMENTS_STORE[request.documentId]["status"] = "CONFIRMED"
...
async def final_save(request: FinalSaveRequest):
    for item in request.academicData:
        FINAL_ACADEMIC_DATA.append({
            "student_id": request.studentId,
            "subject_id": item.subjectId,
            "score": item.score
        })
        
    return {"message": "Academic data saved successfully (In-Memory)", "total_records": len(FINAL_ACADEMIC_DATA)}
